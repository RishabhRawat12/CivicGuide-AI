...
export default function BoothPage() {
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      ...
      <motion.div variants={item} className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {QUICK_FACTS.map((fact, i) => (
          <div key={i} className="glass-card-static p-4 text-center">
            <span className="text-2xl block mb-1" role="img" aria-label={fact.label}>{fact.icon}</span>
      ...
      <motion.div variants={item} className="glass-card p-5 sm:p-6" aria-labelledby="etiquette-title">
        <h2 id="etiquette-title" className="text-base font-bold text-text-primary mb-1 flex items-center gap-2">
          <span role="img" aria-label="Clipboard">📋</span> Polling Booth Etiquette
        </h2>
          ...
          {BOOTH_ETIQUETTE.map((rule, i) => (
            <motion.div key={i}
              ...
              <div className="flex items-start gap-3">
                <span className="text-xl flex-shrink-0 mt-0.5" role="img" aria-label={rule.title}>{rule.icon}</span>
                <div>
      ...
      <motion.div variants={item} className="glass-card p-5 sm:p-6" aria-labelledby="process-title">
        <h2 id="process-title" className="text-base font-bold text-text-primary mb-1 flex items-center gap-2">
          <span role="img" aria-label="Ballot Box">🗳️</span> Voting Process — Step by Step
        </h2>
        ...
        <div className="space-y-3" role="list">
          {[
            ...
            <motion.div key={i}
              ...
              } transition-all`} aria-label={`Step ${s.step}`}>
                {s.step}
              </div>
      ...
      {/* Important Note */}
      <motion.div variants={item} className="p-4 rounded-xl bg-primary/5 border border-primary/15" role="note">
        <p className="text-xs text-text-secondary leading-relaxed">
          <span className="font-semibold text-primary" role="img" aria-label="Info">ℹ️</span> Note: Booth details are generated based on ECI guidelines. 
          For the most accurate booth location, use the official{' '}
          <a href="https://electoralsearch.eci.gov.in/" target="_blank" rel="noreferrer" 
            className="text-primary underline hover:text-primary-glow"
            aria-label="Official Electoral Search Portal (opens in new tab)">
            Electoral Search Portal
          </a>
          {' '}or contact the ECI helpline at{' '}
          <a href="tel:1950" className="text-primary underline hover:text-primary-glow" aria-label="Call ECI Helpline 1950">1950</a>.
        </p>
      </motion.div>
    </motion.div>
  );
