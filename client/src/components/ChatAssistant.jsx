...
export default function ChatAssistant({ fullHeight = false }) {
  ...
  return (
    <div className={`glass-card p-6 ${fullHeight ? 'flex flex-col h-full' : ''}`} role="region" aria-label="AI Voting Assistant">
      <div className="flex items-center justify-between mb-4">
        <h2 className="section-title mb-0">
          <FiMessageCircle className="text-primary" aria-hidden="true" /> AI Chat Assistant
        </h2>
        <button 
          ...
          <span className="sr-only">{voiceOn ? 'Disable voice response' : 'Enable voice response'}</span>
        </button>
      </div>
      ...
      {/* Messages */}
      <div 
        className={`${fullHeight ? 'flex-1' : 'h-72'} overflow-y-auto space-y-3 mb-4 pr-2 flex flex-col`}
        role="log"
        aria-live="polite"
