  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      ...
      <motion.div variants={item} className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-2 flex-wrap" role="group" aria-label="Zone Filter">
          {['All', ...Object.keys(ZONES)].map(zone => (
            <button key={zone} onClick={() => { setSelectedZone(zone); setSelectedState(null); }}
              ...
              {zone === 'All' ? <><span role="img" aria-label="Globe" className="mr-1">🌐</span> All</> : zone}
            </button>
          ))}
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div variants={item} className="lg:col-span-1 h-full overflow-y-auto pr-2 custom-scrollbar flex flex-col gap-4">
          <AnimatePresence mode="wait">
            {selectedState ? (
              <motion.div key={selectedState}
                ...
                role="region" aria-labelledby="state-detail-title">
                <div className="flex items-center justify-between mb-5">
                  <div className="flex items-center gap-3">
                    <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${ZONE_COLORS[getZoneForState(selectedState)].bg} flex items-center justify-center text-xl`} role="img" aria-label={STATES_DATA[selectedState].isUT ? 'Union Territory' : 'State Map'}>
                      {STATES_DATA[selectedState].isUT ? '🏝️' : '🗺️'}
                    </div>
                    <div>
                      <h3 id="state-detail-title" className="text-lg font-bold text-text-primary">{selectedState}</h3>
                      <p className="text-xs text-text-muted">Capital: {STATES_DATA[selectedState].capital}</p>
                    </div>
                  </div>
                  <button onClick={() => setSelectedState(null)} className="text-text-muted hover:text-text-primary" aria-label="Close details">✕</button>
                </div>

                <div className="space-y-3">
                  {[
                    { label: 'Lok Sabha Seats', value: STATES_DATA[selectedState].lokSabha, icon: '🏛️', iconLabel: 'Parliament' },
                    { label: 'Rajya Sabha Seats', value: STATES_DATA[selectedState].rajyaSabha, icon: '🏢', iconLabel: 'Office' },
                    { label: 'Assembly Seats', value: STATES_DATA[selectedState].assemblies || 'N/A', icon: '🏗️', iconLabel: 'Building' },
                    { label: 'Total Voters', value: STATES_DATA[selectedState].voters, icon: '🗳️', iconLabel: 'Ballot' },
                    { label: 'Election Phases', value: STATES_DATA[selectedState].phases, icon: '📅', iconLabel: 'Calendar' },
                    { label: 'Zone', value: getZoneForState(selectedState), icon: '🌐', iconLabel: 'Globe' },
                  ].map((item, i) => (
                    <div key={i} className="flex items-center justify-between py-2.5 border-b border-border/50 last:border-0">
                      <span className="text-sm text-text-secondary flex items-center gap-2">
                        <span className="text-base" role="img" aria-label={item.iconLabel}>{item.icon}</span> {item.label}
                      </span>
                ...
                <a href={`https://eci.gov.in/state/${STATES_DATA[selectedState].abbr.toLowerCase()}`}
                  ...
                  aria-label={`View ${selectedState} on ECI Portal (opens in new tab)`}>
              ...
              <motion.div key="empty"
                ...
                <span className="text-5xl block mb-4" role="img" aria-label="Pin">📍</span>
          ...
          {/* Quick Links Section */}
          <motion.div variants={item} className="glass-card-static p-5 flex-shrink-0" aria-labelledby="quick-links-title">
            ...
            <div className="space-y-2">
              {[
                ...
                <a key={i} href={link.url} target="_blank" rel="noreferrer" 
                  ...
                  <span className="text-base" role="img" aria-label="Link Icon">{link.icon}</span>
      ...
      {/* Zone Legend */}
      <motion.div variants={item} className="glass-card-static p-4" aria-labelledby="legend-title">
      ...
      <motion.div variants={item} className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          card.isCall ? (
            <a key={i} href="tel:1950" className="glass-card-static p-4 text-center hover:border-primary/30 transition-all" aria-label={`Call ECI Helpline ${card.value}`}>
              <span className="text-2xl block mb-1" role="img" aria-label={card.label}>{card.icon}</span>
              <p className={`text-lg font-bold ${card.color}`}>{card.value}</p>
              <p className="text-[10px] text-text-muted mt-0.5">{card.label}</p>
              <p className="text-[9px] text-primary mt-1" aria-hidden="true">Tap to Call</p>
            </a>
          ) : (
