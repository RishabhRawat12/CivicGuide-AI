        ...
        <div className="glass-card overflow-hidden">
          ...
          <div className="p-8">
            ...
            <div className="text-center mb-7">
              <div className="w-14 h-14 rounded-2xl bg-bg-elevated flex items-center justify-center mx-auto mb-3 text-xl shadow-xl shadow-primary/20" role="img" aria-label="Profile setup icon">
                👤
              </div>
            ...
            <div className="flex items-center gap-3 p-3 rounded-xl bg-bg-elevated border border-border mb-6">
              ...
              <span className="text-xs px-2.5 py-1 rounded-full bg-secondary/10 text-secondary border border-secondary/20 flex-shrink-0" role="status">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label htmlFor="setup-age" className="block text-xs text-text-secondary mb-1.5 font-medium">
                    Age <span className="text-accent" aria-hidden="true">*</span>
                  </label>
                ...
                <div>
                  <label htmlFor="setup-pincode" className="block text-xs text-text-secondary mb-1.5 font-medium">
                    <FiMapPin className="inline mr-1" size={12} aria-hidden="true" /> Pincode
                  </label>
              ...
              <div>
                <label htmlFor="setup-state" className="block text-xs text-text-secondary mb-1.5 font-medium">
                  State / UT <span className="text-accent" aria-hidden="true">*</span>
                </label>
                <select id="setup-state" className="input-field" required aria-required="true"
              ...
              <div className="space-y-2 pt-1" role="group" aria-labelledby="checkbox-group-label">
                <p id="checkbox-group-label" className="sr-only">Additional Preferences</p>
                <div className="flex items-center gap-2.5 group">
                  <input id="setup-has-voter-id" type="checkbox" className="w-4 h-4 accent-primary rounded cursor-pointer"
                    checked={formData.hasVoterId}
                    onChange={e => setFormData({ ...formData, hasVoterId: e.target.checked })} />
                  <label htmlFor="setup-has-voter-id" className="text-text-secondary text-xs cursor-pointer group-hover:text-text-primary transition-colors">
                    I have a Voter ID (EPIC) card
                  </label>
                </div>
                <div className="flex items-center gap-2.5 group">
                  <input id="setup-is-first-time" type="checkbox" className="w-4 h-4 accent-primary rounded cursor-pointer"
                    checked={formData.isFirstTimeVoter}
                    onChange={e => setFormData({ ...formData, isFirstTimeVoter: e.target.checked })} />
                  <label htmlFor="setup-is-first-time" className="text-text-secondary text-xs cursor-pointer group-hover:text-text-primary transition-colors">
              ...
              <motion.button type="submit" disabled={submitting}
                ...
                {submitting ? (
                  ...
                  <span className="flex items-center justify-center gap-2">
                    Start My CivicGuide Journey <FiArrowRight size={16} aria-hidden="true" />
                  </span>
                )}
              </motion.button>
            </form>
