...
const TRANSLATIONS = {
  'Where is my polling booth?': {
    ...
    ml: 'എന്റെ പോളിംഗ് ബೂത്ത് എവിടെയാണ്?',
  ...
  'How to register as a voter?': {
    ...
    or: 'ଭୋଟର ଭାବେ ପଞ୍ଜିକରଣ କିପරි କରିବେ?',
  ...
  'How to check voter ID status?': {
    ...
    or: 'ଭୋଟର ID ସ୍ଥିତି କିପରି ଯାଞ୍ચ କରିବେ?',
...
export default function LanguageTranslator() {
  ...
  return (
    <div className="glass-card p-5 sm:p-6" role="region" aria-label="Election Phrase Translator">
      <div className="flex items-center justify-between mb-1">
        <h2 className="text-base font-bold text-text-primary flex items-center gap-2">
          <FiGlobe size={16} className="text-primary" aria-hidden="true" /> Election Translator
        </h2>
        <span className="text-[10px] px-2 py-0.5 rounded-full bg-primary/10 text-primary font-medium" role="status">
      ...
      {/* Language Selector */}
      <div className="flex flex-wrap gap-1.5 mb-4" role="group" aria-label="Select Target Language">
        {LANGUAGES.map((lang) => (
          <motion.button
            key={lang.code}
            whileTap={{ scale: 0.92 }}
            ...
            {lang.name}
            <span className="sr-only">({lang.nameEn})</span>
          </motion.button>
        ))}
      </div>
      ...
      {/* Quick Phrases */}
      <div className="mb-4">
        <p className="text-xs text-text-muted mb-2 font-medium">Select a phrase to translate:</p>
        <div className="flex flex-wrap gap-1.5" role="group" aria-label="Select Phrase">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div className="flex items-center gap-2 mb-3 pb-3 border-b border-border/50">
            <span className="text-[10px] px-2 py-0.5 rounded bg-bg-card text-text-muted font-medium" aria-hidden="true">English</span>
            <p className="text-sm text-text-secondary" aria-label="Source text: English">{selectedPhrase}</p>
          </div>
          ...
          <div className="flex items-start justify-between gap-3">
            <p 
              className="text-lg font-semibold text-text-primary leading-relaxed flex-1" 
              dir={selectedLang === 'ur' ? 'rtl' : 'ltr'}
              aria-live="polite"
              aria-atomic="true"
              aria-label={`Translation in ${langInfo?.nameEn}`}
            >
              {translation}
            </p>
            <div className="flex items-center gap-1.5 flex-shrink-0">
              <motion.button
                ...
                aria-label={`Listen to translation in ${langInfo?.nameEn}`}>
      ...
      <div className="mt-4 p-3 rounded-xl bg-primary/5 border border-primary/10" role="note">
        <p className="text-[10px] text-text-secondary leading-relaxed">
          <span className="font-bold text-primary">💡 Tip:</span> Use these translations to communicate with polling officers if needed. Access to materials in regional languages is your right.
        </p>
      </div>
    </div>
  );
